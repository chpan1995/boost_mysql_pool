...
int main(int argc, char **argv) {
  ...
  // 绑定到结构体
  {
    ...
    if (result.has_value()) {
      ...
      boost::mp11::mp_for_each<boost::describe::describe_members<
          usernode, boost::describe::mod_any_access>>([&](auto D) {
  ...
  {
    boost::mysql::static_results<usernode> result;
    std::vector<boost::mysql::field> pa{boost::mysql::field("admin"),
                                        boost::mysql::field("1234561")};
    pool->query("SELECT id, username,password,createtime,logintime from user "
                "where username=? AND password=?",
                result, pa);
    if (result.has_value() && result.rows().size() > 0) {
      std::cout << result.has_value() << " ---------- " << result.rows().size()
                << std::endl;
      std::vector<usernode> data = std::move(
          std::vector<usernode>(std::make_move_iterator(result.rows().begin()),
                                std::make_move_iterator(result.rows().end())));
  ...
  // 利用一个tuple查询
  {
    ...
    pool->query("INSERT INTO user(username,password,createtime) values(?,?,?)",
                result, tp);
    if (result.has_value()) {
  ...
  // 多种参数复合封装成  查询
  {
    ...
    boost::mysql::results result;
    pool->query(
        "INSERT INTO user(logintime,username,password,createtime,loginouttime) "
        "values(?,?,?,?,?)",
        result, "2024-04-03 08:37:20", std::make_tuple("123"), arry,
        "2024-04-03 08:37:20", struct_to_tuple(structlogtime));
    if (result.has_value()) {
      if (result.affected_rows() != 0) {
        std::cout << "INSERT success" << std::endl;
      }
  ...
  {
    try {
      auto conn = pool->startTransaction();
      std::array<std::string, 3> arry{"chpan", "123", "2024-04-03 08:37:20"};
      std::tuple<std::string, std::string, std::string> tp =
          std::make_tuple("chpan", "123", "2024-04-03 08:37:20");
      std::tuple<std::string, std::string, std::string> tp2 =
          std::make_tuple("chpan2", "123", "2024-04-03 08:37:20");
      boost::mysql::results result;
      pool->query(
          conn, "INSERT INTO user(username,password,createtime) values(?,?,?)",
          result, tp);
      pool->query(
          conn, "INSERT INTO user(username,password,createtime) values(?,?,?)",
          result, tp2);
